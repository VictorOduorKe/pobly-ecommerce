...
const EditBook = () => {
    ...
    useEffect(() => {
        const fetchBook = async () => {
            ...
            try {
                ...
                const book = res.data.find(b => String(b.id) === String(id));
    ...
    return (
        <div className="max-w-md mx-auto py-8">
            <h2 className="text-2xl font-bold mb-6 text-brown-900">Edit Book</h2>
            <form onSubmit={handleSubmit} className="bg-white rounded shadow p-6 flex flex-col gap-4">
                ...
                <button type="submit" className="bg-sienna-500 text-amber-100 px-4 py-2 rounded font-bold hover:bg-brown-900 hover:text-amber-300 transition">Update Book</button>
            </form>
        </div>
    );
};
